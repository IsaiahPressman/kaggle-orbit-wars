...
The input consisted of a number of relevant features for planets, comets, and fleets (e.g. radial and Euclidean position, velocity, encoded ship count) each projected separately via an MLP into a shared 768-dimensional representation. I included seventeen additional tokens: four player summary tokens, one global summary token, four actor plan tokens, four value tokens, and four scratch tokens. The global summary token was projected from the observed global features. Projected per-player summary features (total ships, total production, and so on) were added to four learned player tokens. The actor-plan, value, and scratch tokens used learned embeddings. Scratch tokens had no explicit purpose except to serve as a shared global workspace for the attention operation.
...
For each player and planet/comet action entity, that entity's hidden vector was concatenated feature-wise with the player's summary and plan vectors before being passed through separate source and target projections. The source stream was projected down to a single Bernoulli logit for eligible sources, which was sampled from to decide whether or not to launch a fleet. Sources that decided to launch used a self-attention-like formulation for picking their target:
